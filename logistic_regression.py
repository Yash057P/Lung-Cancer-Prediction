...
def load_and_preprocess_data(file_path):
    ...
    symptom_cols = df.columns.drop(['GENDER', 'AGE', TARGET_COLUMN]).tolist()
...
def train_model(df, target_col):
    ...
    print(f"\nModel training complete (Logistic Regression).") # Updated model name
...
def get_user_input(feature_names, symptom_cols):
    ...
    for feature in symptom_cols:
        while True:
            ...
            if val in ['1', '2']:
                user_data[feature] = 1 if val == '2' else 0 # 2 -> 1 (YES), 1 -> 0 (NO)
                break
            else:
